...
#define TESTsetbits(x,p,n,y)                                              \
	answ = setbits(x,(p),(n),y);                                          \
	printf("Ln%3d:   setbits(%s ,%2d,%2d, %s)   ==>   %s\n", __LINE__,    \
		binstr(x,bufA) ,(int)(p),(int)(n), binstr(y,bufB) , binstr(answ,bufC))
	TESTsetbits(0xf00U,WORD_BITS-7,5,0xffU);
	TESTsetbits(0x0U,WORD_BITS-7,5,0xffU);

	printf("\n-----------------------------------------------------\n\n");

#define TESTinvert(x,p,n)                                            \
	answ = invert(x,(p),(n));                                        \
	printf("Ln%3d:   invert(%s ,%2d,%2d)   ==>   %s\n", __LINE__,    \
		binstr(x,bufA) ,(int)(p),(int)(n), binstr(answ,bufC))
	TESTinvert(0XffU, WORD_BITS  , 0); 
	TESTinvert(0XffU, WORD_BITS-9, 9); 
	TESTinvert(0XffU, WORD_BITS-9, 6); 
	TESTinvert(0XffU, WORD_BITS-9, 0); 
	TESTinvert(0XffU, WORD_BITS-7, 7); 
	TESTinvert(0XffU, WORD_BITS-7, 6); 
	TESTinvert(0XffU, WORD_BITS-7, 0); 
	TESTinvert(0XffU, 0          , 6); 
	TESTinvert(0XffU, 0          , WORD_BITS-7); 
	TESTinvert(0XffU, 0          , WORD_BITS); 

	printf("\n-----------------------------------------------------\n\n");

#define TESTrightrot(x,n)                                          \
	answ = rightrot(x,(n));                                        \
	printf("Ln%3d:   rightrot(%s ,%2d)   ==>   %s\n", __LINE__,    \
		binstr(x,bufA) ,(int)(n), binstr(answ,bufC))
	TESTrightrot(0XfffU, WORD_BITS);
	TESTrightrot(0XfffU, WORD_BITS / 2);
	TESTrightrot(0XfffU, WORD_BITS-3);
	TESTrightrot(0XfffU, 3);
	TESTrightrot(0XfffU, 0);
	TESTrightrot(0X1fcU, 4);
	TESTrightrot(0X1fcU, 8);
	TESTrightrot(0X1fcU, 12);
	TESTrightrot(0Xf00U, 3);
	TESTrightrot(0Xf00U, 15);
}
